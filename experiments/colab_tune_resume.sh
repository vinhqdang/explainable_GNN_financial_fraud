...
# VM "b" needs the complete records of VM "a" for the final RTXGNN runs (uploaded, not synced back)
if [ $VM = b ]; then
  [ -s $HERE/../results/colab_live/tuning_rtxgnn.jsonl ] && \
    timeout 120 colab upload -s $S $HERE/../results/colab_live/tuning_rtxgnn.jsonl /content/rtx/results/tuning_rtxgnn.jsonl
  [ -s $HERE/../results/colab_live/tuned_rtxgnn.jsonl ] && \
    timeout 120 colab upload -s $S $HERE/../results/colab_live/tuned_rtxgnn.jsonl /content/rtx/results/tuned_rtxgnn.jsonl
  FILES="$FILES tuned_rtxgnn.jsonl"
fi
