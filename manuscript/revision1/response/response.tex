...
\end{comment}
\resp We added TGAT, TGN, APAN, FRAUDRE and GAS, and also logistic regression, random forest and XGBoost, which are known to be strong on Elliptic. The comparison now contains 16 baselines in four groups (Table~\ref{tab:main}): feature-only models, general GNNs, temporal GNNs (EvolveGCN-O, TGAT, TGN, APAN) and fraud-specific GNNs (CARE-GNN, PC-GNN, GAS, FRAUDRE, SEFraud). All share the same protocol, budget and threshold selection, and all are run with 10 seeds. To rule out that the comparison favours RTXGNN through the choice of hyperparameters, we added an equal-budget tuning study (Section~\ref{subsec:tuning}, Table~\ref{tab:tuned}). RTXGNN and ten baselines (the tree ensembles, GCN, GraphSAGE, GAT, TGAT, TGN, GAS, FRAUDRE and SEFraud) each received 20 configurations, selected by validation AP only and then evaluated with 10 seeds. Tuning improves several baselines, for example the random forest (\tungainrf{} F1) and GAT (\tungaingat), and leaves the conclusions unchanged: the tree ensembles remain significantly more accurate, and RTXGNN (F1 \tunrtxgnnFone) is not significantly different from the tuned GNNs. Because the original implementations target other graph types (multi-relation review graphs, heterogeneous graphs, link prediction on continuous-time graphs), we re-implemented all graph baselines in one code base and document each adaptation (\ref{app:baselines}). We also explain why continuous-time models have little to exploit on Elliptic: every transaction appears in a single snapshot and no edge crosses snapshots (Section~\ref{sec:related}). For the same reason DySAT reduces to a structural attention network, which is represented by GAT. H2-FDetector and DGA-GNN, which appeared in the first version, were not re-run under the new protocol: their central mechanisms (separating homophilic and heterophilic connections per relation, and decision-tree binning of raw attributes) are represented on Elliptic's single-relation graph by CARE-GNN, PC-GNN, FRAUDRE and the tree ensembles. The first-version numbers of these models were obtained under a different protocol and are not comparable with the new results (Section~\ref{subsec:baselines}). The ``state-of-the-art'' claim has been removed.
...
\end{comment}
\resp We agree, and the new ablation (Table~\ref{tab:ablation}, 5 seeds) contains exactly this comparison. HRAPE is replaced by Time2Vec on the same time gaps, and additionally by absolute-time encodings (HRAPE with absolute time, and Time2Vec on absolute time). The ``without HRAPE'' variant keeps the SEAL masks and the explanation objective, so it isolates the temporal encoding. HRAPE is more accurate than Time2Vec (\ablfullFone{} versus \abltimeTwovecFone{} F1; paired $t$-test over 5 seeds, $p=\abltimeTwovecP$, uncorrected, since a Wilcoxon test cannot reach $p<0.05$ with 5 pairs), and encoding absolute time lowers the mean F1 and makes it much less stable across seeds (standard deviation 0.08 against 0.05), which motivated the shift-invariant design. Because this design was adopted during development, we checked that the validation steps alone lead to the same choice: HRAPE has a validation AP of \valAPfull{}, against \valAPtimeTwovec{} for Time2Vec on the same gaps and \valAPhrapeabs{} with the absolute-time encoding (Section~\ref{subsec:ablation}). Against removing the temporal encoding altogether, the gain is smaller (\ablnohrapeDelta{} for the variant without HRAPE, $p=\ablnohrapeP$), as expected: all Elliptic edges connect transactions of the same time step (Section~\ref{subsec:hrape}). On the synthetic benchmark, HRAPE is ahead of both alternatives in the no-decoy setting and slightly ahead in the default setting, where the differences are smaller than the standard deviations; in the default setting TGAT is the strongest model (Table~\ref{tab:synthetic}). The differences are small in the label-noise setting. We have adjusted the claims accordingly.
...
\end{comment}
\resp In the first version the sparsity contribution was reported as 6.37 points in the abstract and 6.47 points in the results, and the HRAPE contribution as 2.53 and 2.51 points. In the revision, no result number is typed by hand. The script \texttt{experiments/analyze.py} reads the per-run records and writes every table and a file of LaTeX macros (\texttt{tables/numbers.tex}), which the abstract, text, conclusions and this letter all use. A number can therefore not differ between sections. The only intended differences are between studies: the full RTXGNN reaches F1 \rtxFone{} in the main comparison (10 seeds), \ablfullFone{} in the primary ablation (5 seeds), \ablGfullFone{} in the secondary ablation (3 seeds, GPU) and \lertxgnnOneZeroZero{} with all labels in the label-efficiency study (5 seeds, GPU). These studies use different seed sets and hardware, each is compared only with its own reference, and this is now stated in Section~\ref{subsec:protocol}.
...
\end{comment}
\resp All Elliptic results are now over 10 seeds (5 for the ablations), reported as mean $\pm$ standard deviation. For every baseline we test the per-seed differences with a two-sided Wilcoxon signed-rank test and apply Holm's correction over the 16 comparisons (Section~\ref{subsec:protocol}). RTXGNN matches the best GNN baseline and is ahead of the other eleven GNNs on F1, significantly so for several of them before correction and for PC-GNN after correction. On AP, it is significantly better than GCN, GAT, CARE-GNN and PC-GNN after correction. XGBoost is significantly better than RTXGNN. We report all of this in Table~\ref{tab:main} and Section~\ref{subsec:main}. The same tests applied after an equal-budget hyperparameter search for RTXGNN and ten baselines give the same picture (Table~\ref{tab:tuned}).
