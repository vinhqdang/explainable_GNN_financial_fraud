...
Thank you for the careful and candid evaluation of our manuscript and for the opportunity to revise it. The reviewers' central concern, shared by the Associate Editor, was that the claims of the manuscript were not supported by the evidence. We agree with this assessment. In addressing it we found errors in our own experimental pipeline, which we report openly below. The revision is therefore not an incremental edit: we rebuilt the implementation, re-ran every experiment under a single documented protocol, and rewrote the manuscript around what the new evidence supports. Section and table numbers below refer to the revised manuscript. All numbers in the manuscript and in this letter are generated automatically from the per-run records released with the code, which also addresses the consistency concerns raised by both reviewers.

\section*{Summary of the main changes}

\begin{enumerate}[leftmargin=*]
\item \textbf{Corrected data pipeline and re-run experiments.} While preparing the revision we found that the preprocessing used for the first submission did not read the official Elliptic time step of each transaction. Time steps were replaced by a random surrogate, so the "temporal" train/validation/test split and the per-time-step analysis of the first version were not valid. We also could not trace the numbers reported for several baselines (EvolveGCN, DySAT, CARE-GNN, PC-GNN, H2-FDetector, DGA-GNN, SEFraud), for the ablation study, for Amazon Computers and for the runtime breakdown to runs in our code base. All of these numbers have been withdrawn. Every result in the revision comes from the released code (\url{https://github.com/vinhqdang/explainable_GNN_financial_fraud}), with the official time steps and the split of the first version (train 1--30, validation 31--34, test 35--49). We apologise for these errors.
\item \textbf{Honest positioning of the results.} With the corrected pipeline RTXGNN reaches F1 \rtxFone{} and AP \rtxAP{} (10 seeds). It is among the strongest GNNs but not significantly better than the best of them, and the random forest and XGBoost are equally strong or stronger. All claims of state-of-the-art performance, of "19.3\% improvement" and of "production readiness" have been removed.
\item \textbf{A redesigned method.} (i) The fidelity loss criticised by Reviewer~1 was replaced by sufficiency and necessity terms on \emph{hard} top-$k$ explanations. (ii) The label-supervised temporal consistency loss was removed. (iii) HRAPE was made shift-invariant: it no longer encodes absolute time, which we show hurts under a temporal split. (iv) The prediction loss was changed from focal loss to class-balanced cross-entropy. (v) Quantile normalisation of the features, fitted on the training steps only, is now applied for all neural models. The pseudocode was rewritten to match the code line by line, and an inference algorithm was added.
...
\end{comment}
\resp We agree with every part of this comment. The 0.5129 figure came from the flawed pipeline described above. The 19.3\% gain was computed against GAT and not against the strongest baseline; relative to the strongest baseline the reported gain was only about 4\%. With the corrected pipeline RTXGNN obtains F1 \rtxFone{}\,$\pm$\,\rtxFoneSd{} (precision \rtxPrec, recall \rtxRec, AP \rtxAP). The best other GNN (\bestgnnName) obtains F1 \bestgnnFone{}; the difference is not significant after Holm correction. The random forest reaches \rfFone{} and XGBoost \xgbFone{}. These values are in the range reported in the literature for this split (e.g.\ \citealp{weber2019anti}). We now state plainly what the score means: at the validation-chosen threshold, RTXGNN misses \rtxMissPct\% of the \nTestIllicit{} illicit test transactions, and \rtxFDRPct\% of its alerts are false (Table~\ref{tab:operational}, which reports alerts, misses and false alerts, as well as precision at fixed alert budgets). We also state that such a model is only usable as a prioritisation aid in a human review process (Section~\ref{subsec:disc_decision}).
...
\end{comment}
\resp We agree and have adopted the distinction proposed by the reviewer (and by \citealp{rudin2019stop}). RTXGNN is now described as a \emph{self-explaining} model whose masks are attributions computed in the forward pass. The terms "interpretable" and "intrinsically interpretable" were removed from the title and the text, and a terminology paragraph was added at the start of Section~\ref{sec:methodology}.
...
\end{comment}
\resp Algorithm~\ref{alg:training} was rewritten from the code and uses exactly the notation of Section~\ref{sec:methodology}. The description of training was also corrected. Training is full-batch on the subgraph induced by the 2-hop neighbourhoods of the labelled training nodes, which is exact for a 2-layer model, so no neighbourhood sampling is used during training. Neighbourhood sampling (fan-out 25 per hop) is used only at inference and is now described separately in Algorithm~\ref{alg:inference}. Both algorithms name the source files that implement them. The claims of the first version about 3 layers, 4-hop sampling and FP16 training on an A100 GPU did not match the code and have been removed. The algorithms are now placed in the method section, not after the references.
...
\end{comment}
\resp We agree, and the new ablation (Table~\ref{tab:ablation}, 5 seeds) contains exactly this comparison. HRAPE is replaced by Time2Vec on the same time gaps, and additionally by the absolute-time encodings (HRAPE with absolute time as in the first version, and Time2Vec on absolute time). The "without HRAPE" variant keeps the SEAL masks and the explanation objective, so it isolates the temporal encoding. The results do not support the claim of the first version. On Elliptic, HRAPE's contribution is small and within seed variation, which is expected because all edges connect transactions of the same time step (Section~\ref{subsec:hrape}). Encoding \emph{absolute} time, as the first version did, reduces test performance clearly, and this finding motivated the shift-invariant redesign. The benefit of the relative temporal encoding is shown only on the synthetic benchmark, where timing is the discriminative signal (Table~\ref{tab:synthetic}). We state this limitation explicitly.
...
\end{comment}
\resp We agree that the sparsity term is an $L_1$ penalty on auxiliary gate activations and now say so explicitly (Section~\ref{subsec:training}). We ran the requested comparison (Table~\ref{tab:regularisation}): the mask sparsity term at two strengths, no sparsity term, and no sparsity term with dropout 0, dropout 0.5 or weight decay $10^{-3}$. The large effect reported in the first version (from single runs on the flawed pipeline) is not reproduced. The differences are within the seed variation. We have therefore removed "explainability as regularisation" from the contributions and state only that the explanation objective does not cost detection performance (Section~\ref{subsec:disc_reg}).
...
\end{comment}
\resp The GPU figures of the first version could not be reproduced and were withdrawn. Section~\ref{subsec:latency} reports new end-to-end measurements on CPU (\cpuName, one and four threads). Each request samples the 2-hop neighbourhood from an in-memory CSR graph, builds the tensors, runs the forward pass and extracts the explanation, and every stage is timed separately. We report p50/p95/p99 latency, throughput and peak memory for batch sizes 1, 32 and 256 (Table~\ref{tab:latency}). On one core, a single transaction is scored and explained in \rtxLatBoneMed{} ms at the median and \rtxLatBoneP{} ms at the 99th percentile, and reading the explanation from the masks takes \rtxLatBoneExp{} ms. Running GNNExplainer on the same model instead takes \rtxgnnexLatBoneMed{} ms per transaction. The 0.4\,ms of SEFraud refers to a different setting (GPU, amortised over large batches, without neighbourhood retrieval), so the two numbers are not comparable. Our own SEFraud re-implementation is measured under the same protocol as RTXGNN.
...
\end{comment}
\resp We removed Amazon Computers and added T-Finance \citep{tang2022rethinking}, a real transaction network of a financial company with 39,357 accounts, 21 million edges and expert fraud labels, using the GADBench splits (Section~\ref{subsec:tfinance}, Table~\ref{tab:tfinance}). The synthetic benchmark was redesigned so that it is no longer trivially separable. The first version shifted the features of fraudulent nodes by two standard deviations in every dimension. The new generator contains decoy cycles that differ from laundering rings only in timing, carries no feature signal by default, and has four harder or easier variants. We now describe the claims as tests of transfer to a second financial dataset and of the temporal mechanism under controlled conditions, not as evidence of general cross-domain generalisation.
...
\end{comment}
\resp All Elliptic results are now over 10 seeds (5 for the ablations), reported as mean $\pm$ standard deviation. For every baseline we test the per-seed differences with a two-sided Wilcoxon signed-rank test and apply Holm's correction over the 16 comparisons (Section~\ref{subsec:protocol}). RTXGNN is not significantly better than the best GNN baselines, and we say so (Section~\ref{subsec:main}).
...
\end{comment}
\resp With the correct time steps the collapse starts after step 42, i.e.\ at step 43, and it affects all models, including the tree ensembles (Figure~\ref{fig:temporal}). It coincides with the dark-market shutdown documented by \citet{weber2019anti}. The number of illicit transactions per step also drops sharply, which makes per-step F1 very noisy. We added a retraining experiment in which the model is refitted before each test step on all steps up to $t-5$ and validated on steps $t-4$ to $t-1$, i.e.\ with a one-step label delay. Retraining recovers part of the loss for both RTXGNN and the random forest (Section~\ref{subsec:temporal}), but it cannot anticipate a new pattern before labels exist. The words "production-ready" have been removed, and Section~\ref{subsec:deployment} describes the monitoring and retraining a deployment would need.
...
\end{comment}
\resp We agree. The abstract and conclusions now report absolute scores, name the strongest GNN baseline, report the tree ensembles, and state that the difference to the best GNN is not significant. No relative improvement figure is used.
...
\end{comment}
\resp Please see our response to comment 1.6. The experiment is repeated with 5 stratified subsets per fraction, and Table~\ref{tab:label_efficiency} and Figure~\ref{fig:label} show means and standard deviations. The non-monotonic behaviour of the first version was an artefact of single runs on the flawed pipeline.
...
\end{comment}
\resp We agree. The case studies (Section~\ref{subsec:cases}, Table~\ref{tab:cases}) now report only the feature index and its documented group (local or aggregated), the mask value, the effect on the score of removing the top-5 features, and the most important neighbouring transactions with their labels. Statements such as "likely unusual amounts, timing patterns" and the hypothetical mapping "Feature 51 = transaction velocity in last 24h" were removed.

\bigskip
We thank the reviewers again. Their comments led us to find and correct serious problems in our work, and we believe the revised manuscript now reports what the evidence supports.
