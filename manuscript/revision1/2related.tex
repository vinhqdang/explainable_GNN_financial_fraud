...
Continuous-time models assume that nodes interact repeatedly over time. In Elliptic each transaction exists in one of 49 snapshots and all edges connect transactions of the same snapshot \citep{weber2019anti}. The memory of TGN and the mailbox of APAN therefore carry no information across snapshots, and time gaps within a snapshot are zero. We nevertheless include TGAT, TGN, APAN and EvolveGCN as baselines, because they are the standard temporal GNNs, and describe their adaptation in \ref{app:baselines}. DySAT's temporal attention runs over the snapshots in which a node appears. Every Elliptic transaction appears in exactly one snapshot, so DySAT reduces to a structural attention network (GAT), which is already a baseline.
...
Table~\ref{tab:related_comparison} compares RTXGNN with the closest methods on properties that can be verified from the methods themselves. We do not include a ``regulatory compliance'' property, because compliance is a property of an institution's process and cannot be verified from a method, and we report ``explanation in the forward pass'' rather than ``real-time explanation'', since latency depends on the deployment.
