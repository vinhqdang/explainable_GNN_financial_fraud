...
\begin{itemize}
\item \textbf{Data.} Elliptic's snapshots contain no cross-snapshot edges and no intra-snapshot timestamps, so the temporal components of RTXGNN are close to inactive on it. T-Finance has no timestamps at all. The evidence for HRAPE therefore rests on its comparison with Time2Vec on Elliptic and on synthetic data. Validation on a real transaction dataset with continuous timestamps and repeated account activity is the most important next step.
\item \textbf{Baselines.} All graph baselines are our re-implementations under one protocol and budget, and some were adapted from multi-relation or continuous-time settings. They may perform better with their original code. The equal-budget tuning study (Section~\ref{subsec:tuning}) covers ten of the 16 baselines with 20 configurations each; a larger search could change individual results.
\item \textbf{Explanations.} Faithfulness is measured with perturbation metrics that replace features by the training mean. Such perturbations can move inputs off the data manifold. Our training objective also uses perturbations of this kind, which may favour RTXGNN on these metrics. We mitigate this by applying the post-hoc explainers to the same trained model and by reporting a random-explanation reference. The feature masks are nearly binary and weakly necessary, so they identify a set of features the prediction may depend on rather than a ranked list of indispensable ones. No user study has been carried out.
\item \textbf{Compute.} All experiments were run on a 4-vCPU machine and a single T4 GPU. Training cost is about three times that of the same network without the explanation objective. Scalability to graphs with billions of edges was not evaluated.
\end{itemize}
