...
HRAPE supplies three kinds of temporal information to SEAL. In contrast to the first version of this work, it depends only on time \emph{differences} and on counts in trailing windows, never on the absolute timestamp. A model evaluated on future time steps then never sees an out-of-range time value. Section~\ref{subsec:ablation} shows that encoding the absolute timestamp, as the first version did, reduces test performance markedly under the temporal split.
...
\paragraph{What HRAPE can and cannot do on Elliptic} In Elliptic every edge connects two transactions of the same time step and each transaction appears in exactly one time step \citep{weber2019anti}. Hence $\Delta_{uv}=0$ for all edges: the gap code is a constant, $\rho_{uv}=1$, and the velocity vector reduces to the node's in-degree within its time step. On Elliptic HRAPE therefore contributes little beyond a degree feature, and our ablation (Section~\ref{subsec:ablation}) confirms this. The temporal mechanisms become informative only on data with continuous timestamps. We test this on the synthetic benchmark (Section~\ref{subsec:synthetic}), where laundering rings differ from decoy cycles only in the timing of their hops. The hour-of-day, day-of-week and inter-transaction velocity features described in the first version of this manuscript cannot be computed from the public Elliptic data and have been removed.
...
\end{align}
The constant $+2$ initialises the mask close to one, so that training starts from the unmasked model. Unlike the first version, the feature mask is applied only to the raw input features, where it is interpretable as an attribution over named (or at least identifiable) inputs, and not to hidden representations.
...
\label{eq:pred_loss}
\end{equation}
where $\ell$ is the log-loss and $\pi$ the fraction of illicit labels among the training nodes $\mathcal{V}_{tr}$. The focal loss of the first version is kept as an ablation. Its values are two orders of magnitude smaller than those of the explanation terms below, which made the weighting of the objective fragile.
...
\paragraph{Why the objective cannot be met by ignoring the masks} The first version minimised $\mathrm{KL}(f(\mathcal{G})\Vert f(\mathcal{G}_{masked}))$ with soft masks only, as Reviewer~1 pointed out. That objective is minimised trivially by masks that are all close to one, or by a model that is insensitive to the masks, and it says nothing about whether the masks are faithful. The revised objective rules out both degenerate solutions:
...
\paragraph{Removed term} The first version contained a temporal consistency loss that rewarded high temporal weights on edges incident to fraudulent nodes and low weights elsewhere. Because it supervised an internal gate directly with training labels, it could leak label information into the explanation and was not justified. It has been removed.
