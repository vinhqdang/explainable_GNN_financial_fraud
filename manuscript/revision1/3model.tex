...
HRAPE supplies three kinds of temporal information to SEAL. It depends only on time \emph{differences} and on counts in trailing windows, never on the absolute timestamp, so a model evaluated on future time steps never sees an out-of-range time value. Section~\ref{subsec:ablation} shows that additionally encoding the absolute timestamp reduces test performance under the temporal split.
