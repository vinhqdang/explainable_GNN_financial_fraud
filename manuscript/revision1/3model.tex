...
\begin{tikzpicture}[font=\small,
  bx/.style={draw, rounded corners=2pt, align=center, inner sep=4pt, minimum height=12mm},
  ar/.style={-{Latex[length=2mm]}, thick}]
\node[bx, fill=gray!10, text width=3.2cm] (x) at (0,0) {node features $\mathbf{x}_v$};
\node[bx, fill=gray!10, text width=3.2cm] (e) at (0,-2.6) {transactions $(u\!\to\!v,\,t_{uv})$,\\both directions};
\node[bx, fill=orange!10, text width=3.8cm] (fm) at (4.6,0) {\textbf{feature mask}\\$\mathbf{M}^{f}_v=\sigma(g_f(\mathbf{x}_v))$};
\node[bx, fill=blue!8, text width=3.8cm] (hr) at (4.6,-2.6) {\textbf{HRAPE}: time-gap code $\mathbf{e}^{\tau}_{uv}$,\\recency $\rho_{uv}$, velocity $\mathbf{c}_v$};
\node[bx, text width=5.6cm] (h0) at (10.5,0) {input embedding $\mathbf{h}^0_v$\\from $\mathbf{x}_v\odot\mathbf{M}^f_v$ and $\mathbf{c}_v$};
\node[bx, fill=orange!10, text width=5.6cm] (seal) at (10.5,-2.6) {\textbf{SEAL layers} $\ell=1,\dots,L$: edge mask $M^e_{uv}$, node mask $M^n_u$, attention $\alpha_{uv}$ with recency bias; messages gated by $\alpha_{uv}M^e_{uv}M^n_u$};
\node[bx, fill=green!8, text width=5.6cm, below=7mm of seal] (out) {\textbf{prediction} $\hat y_v$ from $[\mathbf{h}^0_v\,\|\,\mathbf{h}^L_v]$;\\\textbf{explanation} from the same pass: top-$k$ features of $\mathbf{M}^f_v$, top-$k$ edges of $M^eM^n\bar\alpha$};
\node[bx, fill=red!6, text width=7.6cm, left=10mm of out] (loss) {\textbf{training only}: $\mathcal{L}=\mathcal{L}_{pred}+w(e)\,[\lambda_{suf}\mathcal{L}_{suf}+\lambda_{nec}\mathcal{L}_{nec}+\lambda_{sp}\mathcal{L}_{sp}]$\\$\mathcal{L}_{suf}$: top-$k$ explanation alone $\Rightarrow$ same prediction\\$\mathcal{L}_{nec}$: its complement alone $\Rightarrow$ uninformative prediction};
\draw[ar] (x) -- (fm); \draw[ar] (fm) -- (h0); \draw[ar] (e) -- (hr); \draw[ar] (hr) -- (seal);
\draw[ar] (h0) -- (seal); \draw[ar] (seal) -- (out);
\draw[ar] (hr.north) -- (h0.south west);
\draw[ar, dashed] (out) -- (loss);
\end{tikzpicture}}
...
\begin{align}
\mathbf{q}_v&=\mathbf{W}_Q\mathbf{h}^{\ell-1}_v,\qquad
\mathbf{k}_{uv}=\mathbf{W}_K\mathbf{h}^{\ell-1}_u+\mathbf{E}^{K}_{r_{uv}}+\mathbf{e}^{\tau}_{uv},\nonumber\\
