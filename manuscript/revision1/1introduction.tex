...
On Elliptic, RTXGNN reaches a test F1 of \rtxFone{} and an AP of \rtxAP{}. This is on par with the best graph neural network baseline, GAS (F1 \gasFone), and the differences to most other GNNs are within the variation across seeds. On average RTXGNN is ahead of the self-explaining SEFraud (\deltaFOnesefraud{} F1, \deltaAPsefraud{} AP), although neither difference remains significant after correction for multiple comparisons. Gradient-boosted trees on the same engineered features remain the most accurate model (XGBoost, F1 \xgbFone), and the random forest reaches \rfFone. For decision support the ranking is what matters: the 500 highest-scored test transactions have a precision of \rtxPrecAtFive{} (Section~\ref{subsec:operational}). Reading the explanation from the masks adds \rtxLatBoneExp{} ms to a single-transaction request. The explanations' sufficiency is close to that of Integrated Gradients, which is about one hundred times more expensive, and they are far more stable than GNNExplainer (Section~\ref{subsec:explanations}). On a synthetic benchmark with planted laundering rings, the most important transaction identified by RTXGNN is a ring transaction for \ringHitPct\% of the detected ring members (Section~\ref{subsec:synthetic}). All models fail after the dark-market shutdown in the Elliptic test period, and retraining with a one-step label delay recovers part of the loss (Section~\ref{subsec:temporal}). End-to-end scoring and explanation of a single transaction, including neighbourhood sampling, takes \rtxLatBoneP{} ms at the 99th percentile on one CPU core (Section~\ref{subsec:latency}).

\subsection*{Contributions}

\begin{enumerate}
\item \textbf{A self-explaining temporal GNN with a faithfulness objective.} RTXGNN gates message passing with learned feature, edge and node masks, and it is trained with explicit sufficiency and necessity terms. These terms rule out masks that are not used by the model. HRAPE uses only shift-invariant temporal information. In our ablation it is more accurate than Time2Vec on the same time gaps, and adding the absolute timestamp makes performance less stable across seeds under a temporal split.
\item \textbf{A reproducible, statistically controlled comparison.} We compare 16 baselines, including tree ensembles and temporal GNNs, under one protocol with 10 seeds and Holm-corrected paired tests, and we release all code. An equal-budget hyperparameter search for RTXGNN and ten baselines, selected on validation data only, leaves the conclusions unchanged. The evaluation includes ablations that replace HRAPE with Time2Vec, a comparison of mask sparsity with dropout and weight decay, stratified label-efficiency experiments with repeated subsets, a per-time-step drift analysis with a retraining strategy, and measured end-to-end CPU latency with tail percentiles.
\item \textbf{An evaluation of explanations for decision support.} We measure the sufficiency and necessity of feature explanations at several sizes against saliency, Integrated Gradients, GNNExplainer, SEFraud's masks and random explanations, together with class-wise results, stability and cost. We test edge explanations against known ground truth on synthetic data, translate detection scores into alert-queue terms, and discuss what is needed to use such explanations in regulated review processes.
\end{enumerate}
